  ...
  angular.module('angucomplete-alt', []).directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', '$interpolate', function ($q, $parse, $http, $sce, $timeout, $templateCache, $interpolate) {
    ...
    function link(scope, elem, attrs, ctrl) {
      ...
      function getLocalResults(str) {
        ...
        for (i = 0; i < scope.localData.length; i++) {
          ...
          for (s = 0; s < searchFields.length; s++) {
            ...
            match = match || (value.toString().toLowerCase().lastIndexOf(str.toString().toLowerCase()) === 0);
          }
          ...
          if (scope.limit && matches.length >= scope.limit) {
            break;
          }
        }
      ...
      function processResults(responseData, str) {
        var i, description, image, text, formattedText, formattedDesc, resultsSize;

        if (responseData && responseData.length > 0) {
          scope.results = [];
          if (scope.limit && responseData.length > scope.limit) {
            resultsSize = scope.limit;
          }
          else {
            resultsSize = responseData.length;
          }
